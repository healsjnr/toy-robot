...
describe "Board" do
  ...
  describe "#place" do
    it "a robot" do
      ...
      expect(board.state).to eq Robot.new(0, 0, north)
    end
 
    it "should allow multiple calls" do 
      board.place(0, 0, :north).place(1, 1, :north).place(3,3, :south)
      expect(board.state).to eq Robot.new(3,3, south)
    end
 
    it "should fail when out of bounds" do
      board.place(-1, 0, :north)
      expect(board.state).to be nil
    end
    
    it "should fail when direction is incorrect" do
      board.place(-1, 0, :elsewhere)
      expect(board.state).to be nil
    end
  end
  ...
  describe "#robot_left and #robot_right" do
    it "should turn the robot left" do
      board.place(0,0,:north).robot_left
      expect(board.state).to eq Robot.new(0,0,west)
    end
    it "should turn the robot right" do
      board.place(0,0,:north).robot_right
      expect(board.state).to eq Robot.new(0,0,east)
    end
    it "should have not effect if the robot has not been placed" do
      board.robot_left
      expect(board.state).to eq nil
